...
public class Encoder extends Thread {
    ...
    /* Writes 33 bytes */
    private final byte[] frame = new byte[Gsm_Def.FRAME_SIZE];
    ...
    public static void stripAUHeader(InputStream in) 
        throws Exception {
          final DataInputStream input = new DataInputStream( (InputStream) in );
	
	    		/* Just read these bits from the stream and do nothing with them */
          final int magic = input.readInt();  	/* magic number SND_MAGIC ((int)0x2e736e64),
																			 		 * which equals ".snd".)
				 		 															 */
          input.readInt();			/* offset or pointer to the data */
          input.readInt();			/* number of bytes of data */
          final int dataFormat = input.readInt();	/* the data format code */
          final int sampleRate = input.readInt();	/* the sampling rate = ~8000 samples/sec. */
    ...
    public void encode(String input_file, String output_file) 
      	...
      	final File arg1 = new File(input_file);
     ...
     * @param to The name of the GSM encoded file.
     */
    public void encode(InputStream input, OutputStream to ) 
			throws IOException {
      ...
      try {
        ...
        {
//        	System.out.println("Encode loop: "+check_stream+", eof: "+input.available() );
           gsm_encode();
      	...
      	IOException ne = new IOException( "Encoder: " + e.getMessage() ); 
      	ne.initCause( e );
      	throw ne;
      } finally {
        if (to != null) {
    ...
    public void encode(InputStream input, String output_file) 
      ...
      try {
        to = new FileOutputStream(output_file);
		encode( input, to );
      } catch (IOException e) {  throw new IOException( "Encoder: " + e.getMessage()); }				
      finally
      {
        if (to != null) try { to.close(); }
           catch (IOException e) { throw new IOException("Encoder: " + e.getMessage()); }		
      }
    }
    ...
    */
    private int ulaw_input(InputStream in) 
			...
			for (; i/2 < input_signal.length && ((c = in.read()) != -1); i+=2) {
				if (c < 0) {
					throw new IOException("Encoder ulaw_input: Corrupt InputStream.");
				}
				else {
					int c2 = in.read();
        			input_signal[i/2] = (c<<8) | c2;//u2s[c];
				}
			}
			return (i/2);
    }
    ...
    private void Gsm_Coder_java() {
        ...
        final short[] ep = new short[40];
        final short[] e  = new short[50];
        final short[] so = new short[160];
    ...
    private void ulaw_output(OutputStream out)
	    throws IOException {
