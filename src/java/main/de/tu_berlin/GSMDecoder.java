...
//    $Id: GSMDecoder.java,v 1.1 2004-05-31 17:59:12 greggwon Exp $	
...
public final class GSMDecoder {
  ...
  public final int[] decode(byte c[]) throws InvalidGSMFrameException {
    ...
    if (c.length!=33)
      throw new InvalidGSMFrameException("GSM Frame not 33 bytes long: "+c.length );

    int i=0;
    
    if (((c[i]>>4) & 0xf) != GSM_MAGIC)
      throw new InvalidGSMFrameException("No GSM_MAGIC at byte "+i+": "+(c[i]&0xff));
