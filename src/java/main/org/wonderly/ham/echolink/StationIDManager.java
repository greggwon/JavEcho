...
/**
 * This class manages the operation of station identification. It listens to
 * events in the system and based on parameters configured by the user, it will
 * cause the appropriate action to identify the station as selected by the
 * parameters.
 */
public class StationIDManager implements LinkEventListener {
	...
	public StationIDManager(Parameters p, Javecho jes) {
		...
		tm.schedule(new TimerTask() {
			public void run() {
				log.fine("Processing ID timer task, lastid: " + new Date(lastIdTime) + ", interval: " + idInterval
						+ ", now: " + new Date() + ", remain: "
				...
				if (lastIdTime + idInterval < System.currentTimeMillis()) {
					try {
						log.fine("Time to ID, whileActive? " + pr.isIdentWhileactive() + ", conn count: "
								+ je.getConnectCount());
						log.fine("whileInActive? " + pr.isIdentWhileinactive() + ", conn count: "
								+ je.getConnectCount());
	...
	private void doProcessEvent(LinkEvent ev) throws IOException {
		...
		if (ev.getType() != LinkMode.NETDATA_EVENT && ev.getType() != LinkMode.MICDATA_EVENT) {
			log.info("Handle LinkEvent: " + ev);
		}
		switch (ev.getType()) {
		case STATION_CONN_EVENT:
