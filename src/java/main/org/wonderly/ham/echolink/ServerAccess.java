...
/**
 *  This is the class used to talk to the EchoLink director server.
 *  It handles login and logout and retrieval of the directory
 *  content.
 */
public class ServerAccess {
	...
	public Vector<Entry> getList( Vector<String> servers, 
			String use, String[]sites, Socket last ) throws IOException {
		if( use != null ) {
			site = use;
			sites[0] = site;
			switchServer = false;
		...
		if( servers != null && switchServer == true ) {
			site = servers.elementAt( (int) 
				( Math.random() * servers.size() ) );
...
//	   for( int i = 0; i < 3; ++i ) {
		  try {
		  	  log.info("Connecting to "+InetAddress.getByName( srvr_addr )+":"+5200 );
			  Socket s = new Socket() {
			  	public void close() {
			  		log.log( Level.INFO, "Socket closed: "+this, new Throwable("socket closed") );
			  	}
			  };
		  	...
		  	if( ioex == null )
		  		ioex = ex;
		  	log.warning( ex.toString()+": "+srvr_addr+":5200" );
...
//	   }
	...
	/**
	 *  Update our logon status with the indicated message
	 *  and busy status of true.
	 *  @param msg the message to user for the station status
	 *  @throws IOException if server connection error occurs
	 */
	 ...
	 *  and busy status
	 *  @param msg the message to user for the station status
	 *  @param how true if the station is busy
	 *  @throws IOException if server connection error occurs
	 */
	public boolean updateLogon(String msg,
			boolean how) throws IOException {
		location = msg;
		sendLogoff();
		lastInfo = msg;
	...
	/**
	 *  Update our logon status indicating the current busy
	 *  status
	 *  @param conn true if station is busy
	 *  @throws IOException if server connection error occurs
	 */
...
//		log.info("key: "+key);
		...
		String logon = prm.getCallSign() +
			"\254\254"+
			prm.getPassword() +
			"\r"+(conn ? "BUSY" : "ONLINE")+Javecho.myVersion()+"J(" +
			sf.format( c.getTime() ) +
			")\r" +
			location +
			"\r" +
	...
	public boolean sendLogoff() throws IOException {
	  ...
	  if(s == null)
	   ...
	   log.log(Level.FINEST,
	   		"Logging off as: "+prm.getCallSign(),
	   		new Throwable("logoff at") );
...
//	    System.out.println(" Receive error ");
	...
	/**
	 *  Get the list of Entry objects representing all stations
	 *  currently logged into the indicated server
	 */
			...
			if (nRead != 4 || sBeginning.startsWith("%%%") ) {
				...
				if( n > 0 ) {
					...
					throw new IOException(
						"Unexpected EOF on server response: "+
							(sBeginning.startsWith("%%%") ? 
							sBeginning.substring(3)+rest :
							((nRead > 0 ) ? new String(bSourceLen,0,nRead) :
...
//			System.out.println(v.size()+" entries found from server, sorting");
			int cnt = 0;
